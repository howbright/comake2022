...
export class SimpleImage {
    ...
    render() {
      ...
      this.wrapper.classList.add(style.simpleimage);
    ...
    _acceptTuneView() {
      this.settings.forEach((tune) => {
        this.wrapper.classList.toggle(style[tune.name], !!this.data[tune.name]);
      ...
      // !!를 사용하면 boolean값으로 변환할 수 있다.
      // classList.toggle 함수는 class 가 있으면 없애주고, 없으면 넣어준다.
    }
